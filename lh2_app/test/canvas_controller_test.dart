...
void main() {
  group('CanvasController Tests', () {
    ...
    test('Viewport panning works correctly', () {
      ...
      // World should have moved 100 pixels to the right (same direction as pan)
    ...
    test('Item management works correctly', () {
      ...
      // Update item rect
      controller.updateItemRect(
          'test-item', const Rect.fromLTWH(15, 25, 120, 60));
      expect(controller.items['test-item']!.worldRect,
          equals(const Rect.fromLTWH(15, 25, 120, 60)));
    ...
    test('Visible object computation works correctly', () {
      ...
      controller.addItem(CanvasItem(
        ...
        itemType: 'node',
        worldRect:
            const Rect.fromLTWH(0, 0, 100, 100), // Visible (center of viewport)
      ));
    ...
    test('Port compatibility validation logic', () {
      const portOut = CanvasPortSpec(
          portId: 'out-1', direction: 'out', portType: 'dependency');
      const portIn = CanvasPortSpec(
          portId: 'in-1', direction: 'in', portType: 'dependency');
      const portInMismatch =
          CanvasPortSpec(portId: 'in-2', direction: 'in', portType: 'data');

      bool isCompatible(CanvasPortSpec from, CanvasPortSpec to) {
        return from.direction == 'out' &&
            to.direction == 'in' &&
